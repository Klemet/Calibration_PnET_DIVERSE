                ...
                while (end.Ticks > date.Ticks)
                {
                    if (!all_values[ecoregion].ContainsKey(date))
                    {
                        if (date.Year != oldYear)
                        {
							...
							{
								int yearIndex = Globals.ConvertYearToFutureClimateYear(date);

								if (ecoregion.Index >= Climate.Climate.FutureEcoregionYearClimate.Length)
									throw new Exception($"Ecoregion index {ecoregion.Index} out of range. Max: {Climate.Climate.FutureEcoregionYearClimate.Length - 1}");

								if (yearIndex >= Climate.Climate.FutureEcoregionYearClimate[ecoregion.Index].Count)
									throw new Exception($"Year index {yearIndex} (year {date.Year}) out of range for ecoregion {ecoregion.Index}. Max: {Climate.Climate.FutureEcoregionYearClimate[ecoregion.Index].Count - 1}");

								ClimateRegionData.AnnualClimate[ecoregion] = 
									Climate.Climate.FutureEcoregionYearClimate[ecoregion.Index][yearIndex];
							}
							else
							{
								int yearIndex = Globals.ConvertYearToSpinUpClimateYear(date);

								if (ecoregion.Index >= Climate.Climate.SpinupEcoregionYearClimate.Length)
									throw new Exception($"Ecoregion index {ecoregion.Index} out of range. Max: {Climate.Climate.SpinupEcoregionYearClimate.Length - 1}");

								if (yearIndex >= Climate.Climate.SpinupEcoregionYearClimate[ecoregion.Index].Count)
									throw new Exception($"Year index {yearIndex} (year {date.Year}) out of range for ecoregion {ecoregion.Index}. Max: {Climate.Climate.SpinupEcoregionYearClimate[ecoregion.Index].Count - 1}");

								ClimateRegionData.AnnualClimate[ecoregion] = 
									Climate.Climate.SpinupEcoregionYearClimate[ecoregion.Index][yearIndex];
							}
